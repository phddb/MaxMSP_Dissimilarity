...
public class Dissimilarity extends MaxObject {
	...
	/**
	 * For cycling through all files, 
	 * to give subject a sense of the range.
	 */
	public void preview(){
		mode = Mode.PREVIEW;
		initPreview();
	...
	private void initPreview(){
		...
		if(false){//resultsPath.exists()){
			...
			for(int i = 0; i < stimuli.length; i++){
				...
				comparisons.add(new Comparison(stimuli[i],stimuli[i]));
			}
	...
	public void inlet(float rating){		
		if(!done){
			...
			if(mode.equals(Mode.EXPERIMENT)){
